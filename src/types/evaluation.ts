...
// === TAMBAHAN INTERN ===
// Mirror EvaluationEmployee, minus field yang memang khusus Employee
// (employment_type, is_active, deactivated_*) — sesuai shape yang
// dikirim EvaluationResource::intern().
export interface EvaluationIntern {
  id: number;
  npk: string;
  name: string;
  jabatan: string | null;
...
// === TAMBAHAN INTERN ===
export type PendingInternTrigger = EvaluationIntern;

export interface Evaluation {
  id: number;
  employee_id: number | null; // === UBAH: nullable (polymorphic) ===
  intern_id: number | null; // === TAMBAHAN INTERN ===
  department_id: number | null;
  ...
  intern: EvaluationIntern | null; // === TAMBAHAN INTERN ===
  ...
  contract_extensions?: EvaluationContractExtension[];
}
...
// === TAMBAHAN INTERN ===
export interface NotExtendPayload {
  reason?: string | null;
}

export interface EvaluationCreatePayload {
  employee_id?: number | null; // === UBAH: opsional (exclusive dgn intern_id) ===
  intern_id?: number | null; // === TAMBAHAN INTERN ===
  department_id?: number | null;
  department_head_id?: number | null;
  npk?: string | null;
...
export interface ConvertToPermanentPayload {
  notes: string | null;
}

export interface ExtendInternContractPayload {
  start_date: string;
  extend_months: number;
  notes: string | null;
}

export interface LatestPkwtInfo {
  pkwt_number: string | null;
  extend_months: number | null;
