...
//   const canViewPending = can("fptk.approve");
...
const FptkPending: React.FC = () => {
  ...
  // ── Fetch pending approvals via TanStack Query ──
  const { data: responseData, isLoading: loading } = usePendingApprovals({
    page,
    per_page: 15,
  });

  const requisitions: Requisition[] = responseData?.data?.data ?? [];
  const pagination = {
    current_page: responseData?.data?.meta?.current_page ?? 1,
    last_page: responseData?.data?.meta?.last_page ?? 1,
    per_page: responseData?.data?.meta?.per_page ?? 15,
  ...
  const { mutateAsync: reviewFptk, isPending: submitting } = useReviewFptk();
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      await reviewFptk({
        noReq: pendingReject.req.no_req,
        data: {
          action: "rejected",
          rejection_reason: pendingReject.reason,
        },
