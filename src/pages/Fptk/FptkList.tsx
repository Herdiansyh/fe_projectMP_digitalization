...
import ConfirmDialog from "../../components/common/ConfirmDialog";
import { FiTrash2 as FiTrashIcon } from "react-icons/fi";
import { useFptkList, useDeleteFptk } from "../../hooks/queries/useFptkQueries";
...
const FptkList: React.FC = () => {
  ...
  const { data: responseData, isLoading: loading } = useFptkList(cleanParams);
  const requisitions = responseData?.data?.data || [];
  const pagination = {
    current_page: responseData?.data?.meta?.current_page || 1,
    last_page: responseData?.data?.meta?.last_page || 1,
    per_page: responseData?.data?.meta?.per_page || 10,
    total: responseData?.data?.meta?.total || 0,
  };

  // ── TanStack Query: delete mutation ──
  const { mutateAsync: deleteFptk, isPending: deleting } = useDeleteFptk();
  ...
  const handleConfirmDelete = async () => {
    if (!deleteTarget) return;
    try {
      await deleteFptk(deleteTarget);
      toaster.create({
        title: "Success",
        description: `Requisition ${deleteTarget} has been deleted.`,
        type: "success",
      });
      setDeleteTarget(null);
    } catch (error: unknown) {
