...
//   const isSectionHead = roleName === "Section Head";
...
//   // ── Worklist "perlu dibuatkan evaluasi" — khusus Leader/Admin ──
//   const {
...
//   // === TAMBAHAN: badge notif harus mencerminkan "evaluasi yang perlu SAYA
//   // aksi sekarang", bukan cuma worklist Leader. Section Head & Manager
//   // punya evaluasi menunggu approve di current_stage mereka masing-masing;
//   // HR Admin/Admin punya pendingHrDecisions. Dihitung terpisah per tab
//   // Employee/Intern via param `type`, dengan per_page:1 supaya cuma perlu
//   // baca `total` dari pagination tanpa fetch data yang tidak dipakai.
//   const actionStage = isSectionHead
//     ? "section_head"
//     : isManager
//       ? "manager"
//       : null;

//   const { data: employeeActionPage } = useEvaluationList({
//     page: 1,
//     per_page: 1,
//     type: "employee",
//     current_stage: actionStage ?? undefined,
//     // enabled tidak didukung langsung oleh useEvaluationList (bukan param
//     // resmi), jadi kita andalkan actionStage null -> query tetap jalan tapi
//     // current_stage undefined berarti tidak difilter. Untuk role selain
//     // Section Head/Manager kita abaikan hasilnya (lihat employeePendingCount).
//   });
//   const { data: internActionPage } = useEvaluationList({
//     page: 1,
//     per_page: 1,
//     type: "intern",
//     current_stage: actionStage ?? undefined,
...
//   const { data: employeeHrDecisions } = usePendingHrDecisions(
//     { page: 1, per_page: 1, type: "employee" },
//     isAdmin || isHrAdmin,
//   );
//   const { data: internHrDecisions } = usePendingHrDecisions(
//     { page: 1, per_page: 1, type: "intern" },
//     isAdmin || isHrAdmin,
//   );

//   // Badge count per tab, sumbernya beda tergantung role yang sedang login:
//   //  - Leader/Admin murni membuat evaluasi baru -> pendingTriggers (worklist)
//   //  - Section Head/Manager -> evaluasi yang menunggu approve mereka
//   //  - HR Admin -> evaluasi yang menunggu keputusan HR (extend/close/permanent)
//   //  - Admin: gabungan trigger + hr decisions, supaya Admin (yang bisa
//   //    override semua stage) tetap kelihatan kalau ada antrian di HR.
//   const employeePendingCount =
//     isSectionHead || isManager
//       ? (employeeActionPage?.total ?? 0)
//       : isHrAdmin
//         ? (employeeHrDecisions?.total ?? 0)
//         : isAdmin
//           ? employeePendingTriggers.length + (employeeHrDecisions?.total ?? 0)
//           : employeePendingTriggers.length;

//   const internPendingCount =
//     isSectionHead || isManager
//       ? (internActionPage?.total ?? 0)
//       : isHrAdmin
//         ? (internHrDecisions?.total ?? 0)
//         : isAdmin
//           ? internPendingTriggers.length + (internHrDecisions?.total ?? 0)
//           : internPendingTriggers.length;

//   // Tabel "Needs Evaluation" di bawah ini TETAP worklist Leader/Admin saja
//   // (bukan diganti) — badge di tab adalah penomoran terpisah, sedangkan
//   // tabel ini tetap menampilkan trigger untuk membuat evaluasi baru.
...
import { evaluationListTourSteps } from "../../hooks/tours/evaluationListTour";
...
const EvaluationList: React.FC = () => {
  ...
  // ─── Tour Guide ──────────────────────────────────────────────────────────
  const canSeeWorklist = isLeader || isAdmin;
  const listTourSteps = evaluationListTourSteps({ canSeeWorklist });
  const tour = useTourGuide("evaluation_list_v1", listTourSteps);
    ...
    <MainLayout>
      <Box>
        ...
        {(isLeader || isAdmin) && (
          <Box
            ...
            p={6}
            mb={6}
            data-tour="needs-evaluation-table"
          >
            <HStack mb={4} gap={2}>
              <FiAlertTriangle color="#c2410c" size={16} />
              <Text fontSize="16px" fontWeight="700" color="gray.800">
        ...
        <Box bg="white" rounded="lg" shadow="sm" p={6}>
          ...
          <HStack mb={5} gap={3} wrap="wrap" data-tour="history-search-filter">
            <Box position="relative" maxW="320px" w="full">
              <Box
                position="absolute"
                left="10px"
          ...
          {loading ? (
            ...
            <Box overflowX="auto" data-tour="history-table">
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ backgroundColor: "#f8fafc" }}>
                    {[
      ...
      {tour.isOpen && tour.currentStep && (
        <SpotlightTour
          ...
          totalSteps={listTourSteps.length}
