...
const EvaluationDetail: React.FC = () => {
  ...
  // approving mencakup updateScores (untuk SH) + approve sekaligus,
  // supaya tombol Approve tetap loading selama kedua langkah berjalan
  const approving = updateScoresMutation.isPending || approveMutation.isPending;
  ...
  // Proses approve yang sesungguhnya — dipanggil langsung kalau tidak perlu
  // konfirmasi tambahan, atau dari dialog konfirmasi setelah user klik "Ya".
  // Dua mutation (updateScores lalu approve) dijalankan berurutan lewat
  // mutateAsync supaya approve baru terkirim setelah skor tersimpan.
  const runApprove = async () => {
    if (!evaluation) return;
    try {
      if (canApprove) {
